...
void
OggDemuxer::InitTrack(TrackInfo::TrackType aTrackType,
...
nsRefPtr<OggDemuxer::InitPromise>
OggDemuxer::Init()
{
...
bool
OggDemuxer::SetupTargetTheora(TheoraState* aTheoraState)
{
  if (mTheoraState) {
    mTheoraState->Reset();
  }
...
bool
OggDemuxer::SetupTargetVorbis(VorbisState* aVorbisState)
{
  if (mVorbisState) {
    mVorbisState->Reset();
  }
  if (!ReadHeaders(aVorbisState, mInfo.mAudio.mCodecSpecificConfig)) {
    return false;
  }
...
void
OggDemuxer::SetupTargetSkeleton(SkeletonState* aSkeletonState)
{
  ...
  if (aSkeletonState) {
    ...
    } else if (ReadHeaders(aSkeletonState, nullptr) && aSkeletonState->HasIndex()) {
    ...
    if (s) {
      if (s->GetType() == OggCodecState::TYPE_THEORA) {
        if (!mHasVideo) {
          ...
          if (SetupTargetTheora(theoraState)) {
            mHasVideo = true;
            continue;
          }
        }
        ...
        if (!mHasAudio) {
          ...
          if (SetupTargetVorbis(vorbisState)) {
...
nsRefPtr<OggTrackDemuxer::SeekPromise>
...
nsRefPtr<OggTrackDemuxer::SamplesPromise>
...
nsRefPtr<OggTrackDemuxer::SkipAccessPointPromise>
